   
    <!-- Section News Single -->
    <section id="section-news-single" class="padding-for-fix-header bg-white">

      <div class="grid-container"> 
        <div class="grid-x align-center">
          <div class="medium-11 cell">

            <div class="grid-container"> 
              <div class="grid-x">

                <div class="xlarge-8 cell">
                  <!-- News Meta -->
                  <div class="news-meta">
                    <span class="section-title">NEWS</span>
                    <span class="category">Press Release</span>
                    <span class="date">08.2018</span>
                  </div>
                  <!-- /News Meta -->
                  <h1 class="h2 page-title">FEDERAL CLEAN FUEL STANDARD STATEMENT</h1>
                </div>

                <div class="small-12 cell">
                  <!-- Featured Image -->
                  <div class="featured-image" style="background-image: url('https://picsum.photos/1600/600?image=655')">
                    <div class="caption">
                      Photo credit: Jack Gill
                    </div>
                  </div>
                   <!-- /Featured Image -->
                </div>

              </div> 
            </div>

            <div class="grid-container">
              <div class="grid-x">

                <!-- Main Content -->
                <div class="large-8 cell">

                  <!-- Page Container -->
                  <div class="page-container">

                    <!-- Page Content -->
                    <div class="page-content">
                      <p>We come together for true progress, in pursuit of a shared vision for Canada⏤a nation poised to rise as a global leader in sustainable abundance. Our mission is to support the Canadian government and stakeholders in catalyzing transformative progress for Canada, to provide policy makers with field expertises and foresights to make informed decisions and take timely actions. </p>

                      <p>Our focus is to accelerate nation-wide adoption of renewable biofuels, biobased products, and clean technologies. Using these technologies can help us get back to the less than 350 parts per mission, which is the threshold that reverses climate change and preserves life as humans have known it.</p>
                      
                      <p>Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas.  Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua.</p>

                      <h2>Heading 2</h2>
                      <h3>Heading 3</h3>
                      <h4>Heading 4</h4>
                      <h5>Heading 5</h5>
                      <h6>Heading 6</h6>
                      
                      <p>Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation <a href="#">ullamco laboris nisi</a> ut aliquip ex eas.</p>

                      <blockquote>
                        <p>WE MUST NOW ESTABLISH THE BASIC PRINCIPLES, THE BASIC VALUES AND BELIEFS WHICH HOLD US TOGETHER AS CANADIANS</p>
                        <div class="author">
                          <strong>Pierre Trudeau</strong> - Former Prime Minister
                        </div>
                      </blockquote>

                      <p>Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas.  Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua.</p>

                      <div class="image-with-caption">
                        <img src="https://picsum.photos/1200/600?image=658" alt="Image 1">
                        <div class="caption">
                          Photo credit: John Doe
                        </div>
                      </div>

                      <p>Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas.  Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua.</p>

                      <p> <img src="https://picsum.photos/1200/600?image=558" alt="Image 1" class="aligncenter">Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas. Lorem ipsum dolor sit amet, con sec te tur adipis cing elit, sed do eius mod tempor incidi dunt ut lab ore et dol ore magna aliqua. Ut enim ad minim ve niamea, nostrud exercitation ullamco laboris nisi ut aliquip ex eas.</p>


                    </div>
                    <!-- /Page Content -->

                    <!-- Page Credit -->
                    <div class="page-credit">
                      <span class="page-credit-title">Article Written by:</span>

                      <div class="page-credit-inner">
                        <div class="page-credit-image">
                          <img src="assets/img/th-andrea-kent.jpg" alt="Andrea Kent">
                        </div>
                        <div class="page-credit-info">
                          <h3>ANDREA KENT</h3>
                          <div>
                            <strong>Greenfield Global, Inc.</strong><br>
                            Vice-President, Government & PR
                          </div>
                        </div>
                      </div>
                    </div>
                    <!-- /Page Credit -->

                    <!-- Page Navigation -->
                    <nav class="page-navigation" role="navigation">
                      <div class="button-previous">
                        <a href="#" rel="prev">Previous Article</a>
                      </div>
                      <div class="button-next">
                        <a href="#" rel="next">Next Article</a>
                      </div>
                    </nav>
                    <!-- /Page Navigation -->

                  </div>
                  <!-- /Page Container -->

                </div>
                <!-- /Main Content -->

                <!-- Sidebar -->
                <div id="sidebar" class="sidebar large-4 cell">

                  <!-- Widget: Newsletter -->
                  <div class="widget newsletter">
                    <?php include('includes/newsletter.php'); ?>
                  </div>
                  <!-- /Widget: Newsletter -->

                  <!-- Widget: Related Posts -->
                  <div class="widget related-posts">
                    <h2 class="h3 all-caps">Related Posts</h2>
                    <?php include('includes/news-feed-small.php');?>
                  </div>
                  <!-- /Widget: Related Posts -->

                </div>
                <!-- /Sidebar -->

              </div>
            </div>


          </div>
        </div>
      </div>
    </section>
    <!-- /Section News -->